...
const links: string[] = ["Home", "Blogs", "About", "Contact"];
...
const Navbar: React.FC = () => {  
  ...
  return (
    <>
      <div className="fixed top-2 left-2 right-2 lg:left-[15rem] lg:right-[15rem] z-50 bg-[#fafafa] rounded-lg drop-shadow-md">
        <nav className="flex justify-between items-center md:p-2 h-16 lg:h-[5rem] mx-4 my-2">
          ...
          <div className="hidden md:flex rounded-3xl text-lg p-2">
      ...
      >
        <div className="bg-[#151515] mt-20 md:mt-0 rounded-3xl w-[60%] ml-auto mr-[5%] text-white p-4 text-sm shadow-[0_8px_30px_rgb(0,0,0,0.12)]">
          <ul className="flex flex-col items-center w-full justify-center gap-6 border-b-[1px] border-gray-500 pb-4 transition-transform duration-300 ">
            {links.map((link) => (
              <Link to={`/${link === "Home" ? "" : link.toLowerCase()}`} className="p-2 px-4 cursor-pointer  transition-colors duration-200 hover:bg-white hover:text-blacks w-full text-center rounded-lg hover:text-black" key={link}>
                {link}
              </Link>
            ))}
          </ul>
